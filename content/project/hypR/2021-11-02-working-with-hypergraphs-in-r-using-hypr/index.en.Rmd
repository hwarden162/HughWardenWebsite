...
For my masters project I studied hypergraphs and their applications in the analysis of biological systems. It was annoying that there was no package for creating hypergraph objects that could have many different analyses applied to them in R or Python. There are many options for traditional graphs (e.g. *igraph* or *NetworkX*) but when it comes to hypergraphs I found that each package could only perform one type of analysis. 

I already had a lot of the code and algorithms for working on hypergraphs that I created for my project. So I defined a hypergraph object and modified my algorithms to work on them. So I ended up with this: **hypR**.
...
The first thing to do is to create a hypergraph object. Hypergraph objects can be created from your own data, but the `example_hype()` function can be used to create many different types of hypergraphs that are useful for exploring hypR.
...
`hype1` is an example of a very basic hypergraph. `hypR` allows for three more ways to augment a hypergraph: weighting, orientation and real coefficients.
...
Hyperedge weightings will change the way information is flows through the hypergraph and so will have an effect on almost every calculation done in `hypR`. Vertex weightings are currently not used in any `hypR` functions except as a store for hyperedge weightings in dual hypergraphs. However, **this is likely to change**, all efforts will be made such that changes should not affect previously written code but this may not be possible/feasible.
...
All efforts have been made to make hypR as flexible as possible. However, although the modelling is extremely flexible not all functions are able to take all forms of hypergaphs, this will be stated in its documentation or as an error message if an execution is attempted. The reason for this is either because the computation is complex and has not been finished yet, or because the maths governing this calculation has not yet been finished. 

If a function does not work for a particular use that you would like, please raise it on the [GitHub Repo](https://github.com/hwarden162/hypR) to move that job higher up the list of features to add.
...
There are currently two methods for creating hypergraphs in hypR, using a hyperedge list or using an incidence matrix.
...
hypR represents oriented hypergraphs using a list of two incidence matrices. The first inidence matrix represents incidence to one end of the oriented hyperedge and the other represents incidence to the other end (it doesn't matter which way round these are). An example is given below
...
This is a vector of weights associated with each vertex. This has little practical use in this hypR implementation except to keep track of hyperedge weights when transforming a hypergraph into its dual.
